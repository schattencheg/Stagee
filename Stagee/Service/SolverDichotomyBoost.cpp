...
vector<double> SolverDichotomyBoost::solve(const ContDiffFunction *f,
...
vector<pair<double, double>> SolverDichotomyBoost::findIntervals(
...
bool SolverDichotomyBoost::sign(cpp_bin_float_quad value) {
  return (value >= 0) ? true : false;
}

vector<double> SolverDichotomyBoost::intervalParse(const ContDiffFunction *f,
                                              pair<double, double> interval) {
  double boundLeft = interval.first;
  double boundRight = interval.second;
  double boundMiddle = (boundLeft + boundRight) / 2.0;
  while ((!isRoot(f, boundMiddle)) || (boundLeft == boundMiddle) || (boundMiddle == boundRight)) {
